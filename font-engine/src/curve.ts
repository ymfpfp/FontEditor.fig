...
export default class Curve {
  ...
  static fromQuadraticBezier(curve: QuadraticBezierCurve) {
    ...
    const controlPoint = (start: [number, number], end: [number, number]) =>
      start.map((value, axis) => value + (2 / 3) * (end[axis] - value)) as [
        number,
        number
      ];
    return new Curve([
  ...
  translate(dx: number, dy: number) {
    return new Curve(
      this.curve.map((p) => [p[0] + dx, p[1] + dy]) as CubicBezierCurve
    );
  }
}
